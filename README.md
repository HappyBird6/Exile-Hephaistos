...
`/`는 PoE2 화폐 창고를 참고한 제작 작업대입니다. 기본 태양의 목걸이를 배치하거나 영문 게임 복사 텍스트를 입력해 아이템 분석 API로 정보를 표시합니다. 미해석 행·경고·전체 원문을 보존하며 결과는 catalog 미검증 초안입니다. 현재 32종 화폐(조각 4종과 감정 주문서 제외)를 우클릭/클릭/키보드로 선택하고 중앙 아이템으로 사용 요청을 확인합니다. Esc 또는 선택 해제로 취소합니다. 제작 효과·옵션 변경·화폐 소모·확률 계산은 수행하지 않습니다. `/admin`, `/admin/crawling`은 기존 관리자 기능입니다. 현재 제작·관리자 화면은 영어만 제공하며 후속 언어 확장은 [Frontend i18n](docs/frontend-i18n.md)을 참고하세요.
