...
1. 사용자와 대화하는 관리자 하나가 구현·테스트·Git 통합을 직접 수행할 수 있다. 고정된 Manager/Test/Review/Git 채팅이나 채팅 이름 기반 역할 판별을 요구하지 않는다.
2. 작은 작업은 관리자가 직접 처리한다. 독립적인 작업으로 나눌 실익이 있고 실제 도구 정책이 허용할 때만 최대 2개 작업자를 사용하며, 작업자는 추가 에이전트를 만들지 않는다.
...
5. 구현 담당은 필요한 테스트의 작성·수정·실행까지 맡는다. 변경 영역과 직접 의존 영역을 우선 검증하고, 영향이 넓거나 프로젝트 필수 절차이면 전체 검증을 선택한다. 전체 검증에 별도 사용자 지시를 기본 요구하지 않는다.
