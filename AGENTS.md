...
## 0. 최우선: 멀티 세션 운영과 채팅 이름 기반 역할 판별

**모든 채팅은 작업 전에 `MULTI_SESSION_WORKFLOW.md`를 읽고, 반드시 자신의 실제 채팅 이름으로 역할을 판별한다. 과거 대화 기억이나 요청 내용만으로 역할을 추정하거나 변경하지 않는다.**

프로젝트 문서의 우선순위는 **`MULTI_SESSION_WORKFLOW.md` → `AGENTS.md` → `docs/TECHNICAL_SPEC.md`(없으면 루트 `TECHNICAL_SPEC.md`) → 승인된 ADR/세부 명세**다. 사용자 명시 지시와 상위 시스템·보안 정책은 별도로 우선한다. 하위 문서의 테스트 의무·완료 조건은 역할별 책임을 뜻하며 기능 채팅에 테스트나 다른 역할의 작업을 허용하지 않는다. `CODEX_SESSION_GUIDE.md`는 이전 운영 방식의 참고 자료이며 이름·역할·초기 프롬프트·작업 순서의 실행 근거로 사용하지 않는다.

### 이름만으로 판별하는 역할

| 실제 채팅 이름 | 역할과 허용 활동 | 결과 보고 대상 |
|---|---|---|
| `Exile-Hephaistos \| Manager` | 기존 담당 채팅에 Git 통합·범위 테스트·리뷰 위임, 결과 취합. 직접 코드 수정·검증·Git 변경 금지 | 사용자 |
| `Exile-Hephaistos \| Test` | 전달받은 변경 범위 테스트 작성·수정·commit·실행. production code 수정 금지 | Manager |
| `Exile-Hephaistos \| Review` | 테스트 성공한 동일 SHA 리뷰. 코드 수정·테스트 실행 금지 | Manager |
| `Exile-Hephaistos \| Git` | branch/worktree 준비, fetch/pull/merge/push. 최종 반영은 사용자 최신화 지시 후 수행 | 요청자 또는 Manager |
| `Exile-Hephaistos \| Feature \| <기능 ID>` | 지정 기능의 production code 구현·자기 변경 commit·기존 담당과 협업. 테스트·리뷰·타 기능 수정·pull/merge/push 금지 | 최초 요청 세션; 최초 세션은 사용자에게 종합 보고 |
| `Exile-Hephaistos \| Other \| <역할 ID>` | 워크플로우에 등록된 기타 역할의 범위만 수행 | 요청자; 직접 지시이면 사용자 |

허용 기능 ID: `season`, `modifier`, `item`, `currency`, `currencyrule`, `price`, `crafting`, `preset`, `ai`, `user`, `frontend`, `data-pipeline`, `api-integration`.
허용 기타 역할 ID: `bootstrap`, `docs`, `architecture`, `db-coordination`, `research`. 상세 소유 경로와 금지 활동은 워크플로우 0.2절을 따른다. Manager/Test/Review/Git은 프로젝트당 활성 채팅을 하나만 둔다.

1. 제공된 앱 문맥 또는 사용 가능한 채팅 조회 도구로 자신의 실제 이름과 세션 ID를 확인한다. 다른 채팅의 이름을 자기 이름으로 간주하지 않는다.
2. 이름을 위 규칙과 정확히 대조하고 등록된 프로젝트·세션 ID를 확인한다. 이름 확인 불가, 미등록 이름, 중복 담당, 이름과 최초 프롬프트 역할 불일치는 `UNCLASSIFIED` 또는 역할 충돌로 사용자에게 보고하고 작업을 시작하지 않는다. 임의 이름 변경·세션 생성으로 우회하지 않는다.
3. 시작 시 `채팅 이름 / 역할 / 허용 범위 / 지정 branch·worktree / 보고 대상`을 짧게 명시한다. 기능별 작업 내용과 시작 SHA는 최초 프롬프트 및 진행 기록으로 확인하되 역할은 이름으로 결정한다.
4. branch/worktree를 직접 지정받으면 해당 공간을 사용한다. 지정이 없으면 각 채팅이 워크플로우 0.3절에 따라 자기 전용 branch/worktree를 별도 사용자 확인 없이 생성하고 기록한다. 자기 소유로 확인된 기존 공간은 재사용한다. 생성은 모든 역할에 허용되는 작업 준비 예외이며 역할별 코드 수정·commit·검증 권한을 확대하지 않는다. 기존 checkout을 전환하거나 사용자 변경을 이동하지 않는다. 준비 후에는 확정된 공간에서만 작업하고 허용된 자기 변경만 commit한다. 지정 불일치·생성 실패는 즉시 보고한다.
5. 다른 기능이 필요하면 기존 담당 채팅에 추가 승인 없이 요청하고 결과를 기다린다. 담당이 없거나 모호하면 사용자에게 보고한다. 협업 상대는 자기 기능만 구현하며 최초 세션이 결과와 commit을 종합한다.
6. 구현 완료 뒤에는 사용자 보고에서 멈춘다. 사용자가 Manager에 검증을 지시해야 Git 통합 → 변경 범위 테스트 → 성공한 동일 SHA 리뷰 → 종합 보고를 진행한다. 최종 merge·push는 사용자의 별도 Git 최신화 지시 후 Manager가 Git에 요청한다.
7. 전체 테스트·전체 build·전체 lint·전체 typecheck는 사용자 명시 지시가 있을 때만 수행한다. 기능 세션은 compile/build도 기본 불허다. 필요한 검사·fixture 항목을 Test에 전달한다.
8. 실패·차단·수정이 필요한 리뷰 결함·무응답 기준 초과 시 해당 작업 흐름을 즉시 중단하고 보고한다. 자동 수정·재시도·우회·검증 생략으로 진행하지 않는다. 구현 단계는 최초 세션이, 검증·최신화 단계는 Manager가 사용자에게 보고한다.
...
1. 먼저 0절에 따라 실제 채팅 이름으로 역할을 판별한다. 그다음 현재 디렉터리, Git 상태, 작업 경로의 추가 지침, README, 관련 기술 명세·ADR·build/lockfile을 자신의 업무에 필요한 범위에서 읽는다. 실행 명령 확인은 실행 허가가 아니다.
2. 변경할 모듈과 공개 계약을 특정한다. 게임 동작 변경이면 `docs/supported-mechanics.md`, `docs/data-sources.md`, 해당 snapshot fixture의 출처부터 확인한다.
3. 기능 세션은 버그 재현 조건, 새 규칙의 출처와 적용/실패 조건, 필요한 fixture·검사 항목을 기록한다. 테스트 작성·실행은 사용자 검증 지시 후 Test 세션이 담당한다.
4. 자신의 소유 범위만 구현한다. schema/API/설정 변경으로 다른 담당의 migration/OpenAPI/문서 갱신이 필요하면 기존 소유 세션과 협업한다. lint/typecheck/test를 구현 세션에서 자동 실행하지 않는다.
5. 완료 보고에는 바뀐 동작, 변경 파일·commit, 미검증 항목과 Test에 전달할 조건, 남은 TODO를 적는다. 역할별 구현·테스트·리뷰·Git 반영 완료를 구분하며 실행하지 않은 검증을 통과했다고 말하지 않는다.

아직 저장소가 scaffold되지 않았으면 아래 구조와 명령은 **구현 목표**다. 없는 script나 endpoint가 이미 존재한다고 보고하지 않는다. 실데이터 없이 adapter 계약 구현은 담당 기능 세션이, synthetic 테스트는 지시받은 Test 세션이 수행할 수 있다.
...
### Test 세션이 변경 범위에 따라 수행할 필수 테스트

이 절과 앞 절의 모든 테스트·회귀·fixture 요구는 제품의 검증 기준이다. 테스트 작성·수정·실행은 Test 세션, build/task 설정은 Bootstrap, 정식 리뷰는 Review의 책임이다. 기능 세션이 이 요구를 근거로 역할 범위를 확대하지 않는다.
...
Test 세션은 모듈 변경에 관련 ArchUnit을 포함하고 버그 변경은 전달받은 재현 조건으로 회귀 테스트를 작성한다. 실패하면 제품 코드를 고치지 않고 Manager에게 즉시 보고한다. mock이 구현을 그대로 복제하는 테스트보다 관찰 가능한 동작을 검증한다. 게임 규칙 fixture는 출처를 포함하고 잘못된 fixture를 맞추려고 구현을 왜곡하지 않는다.

### 초기 scaffold에서 제공할 명령 계약

아래는 프로젝트가 생성된 후 제공할 표준 명령 계약이며 자동 실행 목록이 아니다. Test는 실제 파일/script와 허용 범위를 확인하고 범위 검증 명령을 선택한다. 전체 검사 명령은 사용자 명시 지시가 필요하다. 설정 변경은 Bootstrap, README와 이 표 갱신은 문서 담당에게 전달한다. 아직 실행되지 않은 명령을 성공한 검증으로 보고하지 않는다.
...
`integrationTest` task 연결은 Bootstrap이 Test와 계약을 맞춰 구성한다. Docker 등 필수 환경이 없어 실행 불가하면 해당 검증 흐름을 중단하고 Manager를 통해 사용자에게 BLOCKED를 보고한다. 비용 발생 live smoke나 production 변경은 기본 test에 포함하지 않는다.

### 완료 체크

1. 요청한 동작이 구현되어 있고 직접 입력/AI 경로가 동일 Engine을 사용한다.
2. 변경된 규칙·데이터는 출처와 버전을 갖추고 미지원 항목을 정상처럼 처리하지 않는다.
3. 구현 완료에는 필요한 API/schema/env 문서와 협업 commit이 포함된다. 관련 test/lint/typecheck 통과는 이후 Test 보고로, 리뷰 통과는 Review 보고로 별도 확인한다. 구현 완료를 전체 검증 완료로 표시하지 않는다.
4. diff에 secret·generated noise·요청 밖 리팩터링·사용자 변경 삭제가 없다.
5. Manager는 대상 SHA별 구현·통합·테스트·리뷰·반영 상태와 남은 TODO를 구분한다. 실패·차단 시 해당 작업 흐름을 즉시 멈추고 사용자 지시를 기다린다. 별개 작업의 기존 변경을 임의 정리하지 않는다.
