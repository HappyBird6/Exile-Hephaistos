...
이 프로젝트는 독립 운영 명세인 [MULTI_SESSION_WORKFLOW.md](docs/solo-workflow/MULTI_SESSION_WORKFLOW.md)를 채택한다. 먼저 해당 문서를 읽고, Git 전용 채팅은 연결된 Git 통합 절차도 따른다. 운영 역할과 절차는 그 문서에서만 정의한다.

상위 시스템·보안 정책과 사용자 명시 지시를 우선한다. 운영은 위 명세를, 제품·기술 요구사항은 이 파일 → `docs/TECHNICAL_SPEC.md`(없으면 루트 `TECHNICAL_SPEC.md`) → 승인된 ADR/세부 명세를 따른다. 운영 기본값으로 제품 요구사항을 변경하지 않는다. `CODEX_SESSION_GUIDE.md`는 과거 참고 자료다.
